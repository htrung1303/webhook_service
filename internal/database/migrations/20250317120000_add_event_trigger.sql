...
-- +goose Down
-- +goose StatementBegin
-- Remove the system state table
DROP TABLE IF EXISTS webhook_system_state;

-- Remove the index
DROP INDEX IF EXISTS idx_webhook_events_pending_next_attempt;

-- Remove the trigger and function
DROP TRIGGER IF EXISTS webhook_event_change_trigger ON webhook_events;
DROP FUNCTION IF EXISTS notify_webhook_event_change();
DROP FUNCTION IF EXISTS check_retry_events();
-- +goose StatementEnd 
